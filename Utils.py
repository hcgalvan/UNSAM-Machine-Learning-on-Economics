...
def plot_regions(classifier, x, t, log=False):
    """Plot results from classification with a SVM."""
    plt.figure(figsize=(9, 7))
...
#     plt.pcolormesh(xx, yy, zz, cmap=plt.cm.RdBu_r)
    plt.pcolormesh(xx, yy, zz, cmap=plt.cm.bwr, shading='auto')

    if isinstance(classifier, sklearn.svm.SVC):
        # decision boundary
        plt.contour(xx, yy, -Z, 0, colors='0.5', zorder=1)
        # margins
        plt.contour(xx, yy, -Z, [-1, 1], colors='0.25', linestyles='dashed',
                    zorder=1)
